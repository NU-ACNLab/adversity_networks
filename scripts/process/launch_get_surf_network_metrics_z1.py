...
# Get the subject directories for the subjects in the prior
df = pd.read_csv(tabdir + 'prior_subjects_2026-03-23.csv')
...
for subdir in subdirs:
    ...
    if not os.path.exists(outdir+sub+'/'+ses):
        os.mkdir(outdir+sub+'/'_ses)
    #if not os.path.exists(outdir + 'sub-' + subid + '/ses-1/sub-' +
                            #subid + '_ses-1_surf_network_metrics_z1.csv'):
    cmd = ['Rscript /projects/b1108/projects/adversity_networks/scripts/process/get_surf_network_metrics_z1.R -s', subid, '-e', sesid]
    get_surf_network_metrics_script = launchdir+sub+'_'+ses+'_get_surf_network_metrics_z1_run.sh'
    os.system('cat /projects/b1108/projects/adversity_networks/scripts/process/sbatchinfo_12hr_10G_general.sh > '+get_surf_network_metrics_script)
