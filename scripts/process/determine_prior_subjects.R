...
fd_df <- read.csv(paste0(indir, 'neuroimaging/tabulated/meanFD_2026-03-19.csv')) # N = 227 (length(df[df$sesid == 2, 'sesid']))
clin_df <- read.csv(paste0(indir, 'clinical/clinical_2025-08-11.csv')) # N = 

# Calculate the number of minutes remaining
qual_df$minutes <- ((qual_df$nTRs - qual_df$nRegressors)*2.05)/60

# Merge fd and clin
df <- merge(fd_df, clin_df) # N = 

# Sum the number of minutes
df$minutes <- NA
for (i in 1:nrow(df)) {
    subid <- qual_df$subid == df[i, 'subid']
    sesid <- qual_df$subid == df[i, 'sesid']
    df[i, 'minutes'] <- sum(qual_df[which(qual_df$subid == subid & qual_df$sesid == sesid), 'minutes']) 
}

# Filter out subjects with fewer than 5 minutes remaining
sum(df[df$sesid == 2, 'minutes'] < 5) # number of these subjects = 
# range(df$minutes)... 
#df <- df[df$minutes > 5,]
